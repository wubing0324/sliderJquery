		...
		_bindMouseWheel : function(){
			...
			self.$cont.on("mousewheel DOMMouseScroll",function(e){
				...
				var oEv = e.originalEvent,
					...
					//这里如果一直滚动鼠标轮，那么会一直scrollTo(),到了最下面的时候，也就是滑块儿滚动到最底部的时候，滚动条会根据这句话Math.min(maxSliderPosition,maxSliderPosition * self.$cont[0].scrollTop / self.getMaxScrollPosition());决定是否继续滚动
					//每次滚动鼠标轮，会触发_bindContScroll中的on，on、中又调用self.getSliderPosition()，所以滑块儿滚动到最后，再滚向后动鼠标轮滑块儿也不会动。但是内容是怎么停止的呢，因为此时mousedown.on()函数并没有被触发，即便触发了，在最后mouseup也解除了对函数的绑定。所以这里的函数我们调用不到，那为什么内容会停止滚动呢，因为scrollTop的值到达最大之后，是不会再改变的。例如当前页面中，scrollTop最大值为
					//812，我们设置$(".scroll-cont").scrollTop(8120);是没有用的，打印出$(".scroll-cont").scrollTop();的值，还是812
			});
			return self;
		},
		//计算滑块儿的当前位置
